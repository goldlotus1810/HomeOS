...
// Node cùng cluster → gần nhau; khác cluster → xa nhau
func (t *Tree) projectTo3D(addr isl.Address) [3]float32 {
	// Dùng địa chỉ ISL để tạo vị trí 3D có tính nhất quán
	// Layer → góc quanh trục Y
	// Group → khoảng cách từ tâm
	// Type → cao độ Z

	layerAngle := float64(addr.Layer-'A') / 26.0 * 2 * math.Pi
	groupRadius := 1.5 + float64(addr.Group-'A')/26.0*2.0
	typeHeight := float64(addr.Type-'a') / 26.0 * 2.0

	return [3]float32{
		float32(groupRadius * math.Cos(layerAngle)),
		float32(typeHeight - 1.0),
		float32(groupRadius * math.Sin(layerAngle)),
	}
}
